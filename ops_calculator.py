...
import sys
sys.path.append("/home/mohammad/research/dynamic-nn/repo/Funnel-Injector/")
...
def load_model(model_config, device, distributed):
    if 'custom' in model_config['name']:
        repo_or_dir = model_config.get('repo_or_dir', None)
        model = get_model(model_config['name'], repo_or_dir, **model_config['params'])
    else:
        model = custom.resnet_1d.resnet18()
...
def load_dataset(dataset_directory, snrs=None):
    all_snrs = torch.load(dataset_directory + "snrs.pt")
    dataset = torch.utils.data.TensorDataset(torch.load(dataset_directory + "data.pt"), torch.load(dataset_directory + "label.pt"))
    datasets_snr = {}
    train_ds_snr = {}
    val_ds_snr = {}
    if snrs == None:
    ...
    for snr in snrs:
        datasets_snr[snr] = torch.utils.data.Subset(dataset, (all_snrs == snr).nonzero(as_tuple=True)[0])
        train_ds_snr[snr], val_ds_snr[snr] = torch.utils.data.random_split(datasets_snr[snr], [int(0.8 * len(datasets_snr[snr])), len(datasets_snr[snr]) - int(0.8 * len(datasets_snr[snr]))], generator=generator1)
...
def main(args):
    ...
    train_ds_snr, val_ds_snr = load_dataset(dataset_directory, snrs=config['misc']['snrs'])

    train_ds, val_ds = torch.utils.data.ConcatDataset(train_ds_snr.values()), torch.utils.data.ConcatDataset(val_ds_snr.values())
    dataset_dict = {'radioml2018/train': train_ds, 'radioml2018/val': val_ds}
    ...
    flops, params = tp.utils.count_ops_and_params(student_model, example_inputs=train_ds[0][0].unsqueeze(0).to(device))
    print(f'Student: Flops: {flops}, Params: {params}')

    flops, params = tp.utils.count_ops_and_params(student_model, example_inputs=train_ds[0][0].unsqueeze(0).to(device), ignore_modules=list(student_model.modules())[10:])
    print(f'Head: Flops: {flops}, Params: {params}')

    flops, params = tp.utils.count_ops_and_params(student_model, example_inputs=train_ds[0][0].unsqueeze(0).to(device), ignore_modules=list(student_model.modules())[:10])
    print(f'Tail: Flops: {flops}, Params: {params}')
